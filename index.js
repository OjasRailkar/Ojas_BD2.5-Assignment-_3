...
function sortByPriceAscending(hotel1, hotel2) {
  return hotel1.price - hotel2.price;
}

function sortByPriceDescending(hotel1, hotel2) {
  return hotel2.price - hotel1.price;
}

app.get('/hotels/sort/pricing', (req, res) => {
  // get query parameter from query (high to low / low to high)
  let pricing = req.query.pricing;
  let hotelsCopy = hotels.slice();
  let sortedHotels;

  if (pricing === 'high-to-low') {
    sortedHotels = hotelsCopy.sort(sortByPriceDescending);
  } else if (pricing === 'low-to-high') {
    sortedHotels = hotelsCopy.sort(sortByPriceAscending);
  } else {
...
function sortByRatingAscending(hotel1, hotel2) {
  return hotel1.rating - hotel2.rating; //Asc
}

function sortByRatingDescending(hotel1, hotel2) {
  return hotel2.rating - hotel1.rating; //Dsc
}
...
app.get('/hotels/sort/rating', (req, res) => {
  ...
  if (rating === 'high-to-low') {
    sortedHotels = hotelsCopy.sort(sortByRatingDescending);
  } else if (rating === 'low-to-high') {
    sortedHotels = hotelsCopy.sort(sortByRatingAscending);
  } else {
    sortedHotels = hotels;
  }
...
app.get('/hotels/sort/reviews', (req, res) => {
  ...
  if (reviews === 'least-to-most') {
    sortedHotels = hotelsCopy.sort(sortByReviewsAscending);
  } else if (reviews === 'most-to-least') {
    sortedHotels = hotelsCopy.sort(sortByReviewsDescending);
  } else {
    sortedHotels = hotelsCopy;
  }
